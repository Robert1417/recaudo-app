...
sin_mode_active = st.session_state.get("modo_sin_portafolio", False) and len(ids_sel) > 1

if not sin_mode_active:
    ...
    with c3:
        st.number_input("🧮 N PaB", min_value=1, step=1, value=int(st.session_state.n_pab), key="n_pab")
    ...
    if n_pab != prev_n_pab:
        st.session_state.primer_pago_banco = (pago_banco / n_pab) if n_pab > 1 and pago_banco > 0 else pago_banco
    st.session_state._prev_n_pab_for_primer = n_pab

    if n_pab > 1:
        pago_banco_actual = float(st.session_state.pago_banco or 0.0)
        st.session_state.primer_pago_banco = min(max(float(st.session_state.get("primer_pago_banco", 0.0) or 0.0), 0.0), pago_banco_actual)
        pesos_input("💳 Primer PAGO BANCO", key="primer_pago_banco", help="Monto del primer pago al banco (el resto se reparte en los siguientes PaB).")
    else:
    ...
    with c4:
        ...
        new_ce = pesos_input("🏁 Comisión de éxito (editable)", key="comision_exito", help="Por defecto se calcula con la fórmula, pero puedes ajustarla manualmente.")
        if not st.session_state.get("comision_exito_overridden", False) and abs(new_ce - auto_ce) > 0.5:
    ...
    if ce_inicial <= 0:
        ...
        st.progress(int(round(max(0.0, min(porcentaje, 100.0)))))
        st.caption(f"CE inicial: {ce_inicial:,.0f}  |  Comisión de éxito: {base:,.0f}  →  **{porcentaje:,.2f}%**")
else:
    st.info("Sin portafolio activo: los inputs de PAGO BANCO, N PaB, CE y PLAZO se configuran por deuda en cada bloque.")
...
if st.session_state.get("modo_sin_portafolio", False) and len(ids_sel) > 1:
    ...
    for i, deuda_row in deuda_detalle.iterrows():
        ...
        com_total_key = f"sin_port_ce_total_edit_{ref_input}_{deuda_id}"
        ...
        primer_key = f"sin_port_primer_banco_{ref_input}_{deuda_id}"
        ...
        comision_auto_i = max((deuda_resuelve_i - pago_i) * 1.19 * ce_base_mode, 0.0)
        if com_total_key not in st.session_state:
            st.session_state[com_total_key] = float(comision_auto_i)
        comision_i = max(float(_parse_amount_input(st.session_state.get(com_total_key, comision_auto_i))), 0.0)
        ...
        with r2:
            st.number_input(f"Deuda Resuelve · {deuda_id}", min_value=0.0, value=deuda_resuelve_i, step=0.0, format="%.0f", disabled=True, key=f"sin_port_deuda_res_{ref_input}_{deuda_id}")
        with r3:
            st.number_input(f"PAGO BANCO · {deuda_id}", min_value=0.0, value=pago_i, step=0.0, format="%.0f", key=pago_key)
        with r4:
            st.number_input(f"Comisión Éxito · {deuda_id}", min_value=0.0, value=float(comision_i), step=0.0, format="%.0f", key=com_total_key)
        with r5:
        ...
        primer_sugerido = pago_i if n_pab_i == 1 else (pago_i / n_pab_i if n_pab_i > 0 else 0.0)
        if primer_key not in st.session_state:
            st.session_state[primer_key] = float(primer_sugerido)
        primer_val = min(max(float(_parse_amount_input(st.session_state.get(primer_key, primer_sugerido))), 0.0), pago_i)
        if n_pab_i > 1:
            st.number_input(f"Primer PAGO BANCO · {deuda_id}", min_value=0.0, max_value=float(pago_i), value=primer_val, step=0.0, format="%.0f", key=primer_key)
        else:
            st.session_state[primer_key] = float(pago_i)
        config_rows.append({
            "Id deuda": deuda_id,
            ...
            "N PaB": n_pab_i,
            "PLAZO": plazo_i,
            "Primer Pago Banco": float(st.session_state.get(primer_key, primer_sugerido)),
        })
    ...
    if not default_conf.empty:
        ...
        cuota_apartado = float(np.nansum(cuota_vals)) if cuota_vals else np.nan
        ...
        with k4:
            st.text_input("Cuota/Apartado (suma)", value=("—" if np.isnan(cuota_apartado) else f"{cuota_apartado:.4f}"), disabled=True)
    ...
    for i, row in default_conf.iterrows():
        ...
        primer_banco_i = min(max(float(row.get("Primer Pago Banco", 0.0) or 0.0), 0.0), pago_total_i)
