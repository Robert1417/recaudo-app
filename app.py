...
if st.session_state.get("modo_sin_portafolio", False) and len(ids_sel) > 1:
    ...
    recalc_view = default_conf[["Id deuda", "Pago Banco Total", "Comisión Total"]].copy()
    recalc_view["Pago Banco Total"] = recalc_view["Pago Banco Total"].map(_format_currency0)
    recalc_view["Comisión Total"] = recalc_view["Comisión Total"].map(_format_currency0)
    st.caption("Distribución final aplicada (recalculada):")
    st.dataframe(recalc_view, use_container_width=True, hide_index=True)
    ...
    limite_comision = float(sum(comision_deuda_calc))
