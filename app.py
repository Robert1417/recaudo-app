...
def load_pagos_banco_data(current_year: int) -> pd.DataFrame:
    ...
    return result.loc[:, display_columns].rename(columns=renamed_columns).reset_index(drop=True)
...
def render_pagos_banco_module() -> None:
    ...
    try:
        ...
        st.info("Verifica que `MI_JSON` tenga acceso a los Google Sheets y que las pestañas se llamen `Cartera` y `HC`.")
        st.stop()
    ...
    amount_col = next((col for col in pagos_df.columns if _norm(col) == _norm("Monto")), None)
    if amount_col:
