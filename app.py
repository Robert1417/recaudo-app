...
def _validate_flow_matches_pdf(first_page_text: str, expected_flow_df: pd.DataFrame | None) -> tuple[bool, str]:
    ...
    if not pdf_rows:
        return False, "El flujo no coincide: no se encontró la tabla de flujo en la primera hoja del PDF."

    if len(pdf_rows) != len(expected_rows):
        return (
            False,
            f"El flujo no coincide: la calculadora tiene {len(expected_rows)} filas y el PDF tiene {len(pdf_rows)} filas.",
        )
    ...
    if expected_concepts != pdf_concepts:
        return False, "El flujo no coincide: los conceptos de la primera tabla no son iguales entre calculadora y PDF."
    ...
    if comparable_dates:
        ...
        if expected_dates_norm != pdf_dates_norm:
            return False, "El flujo no coincide: las fechas de la primera tabla no son iguales entre calculadora y PDF."
    ...
    if comparable_amounts:
        for expected_row, pdf_row in zip(expected_rows, pdf_rows):
            if abs(float(expected_row["amount"]) - float(pdf_row["amount"])) > 1.0:
                return False, "El flujo no coincide: los montos de la primera tabla no son iguales entre calculadora y PDF."
