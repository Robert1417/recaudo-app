...
if st.session_state.get("modo_sin_portafolio", False) and len(ids_sel) > 1:
    ...
    alloc_default = pd.DataFrame({
        "Id deuda": deuda_detalle["Id deuda"],
        "Deuda Resuelve": deuda_detalle["Deuda Resuelve"],
        "Pago Banco Total": pago_banco_deuda,
        "N PaB": n_pab_deuda,
    })
    alloc_view = alloc_default.copy()
    alloc_view["Deuda Resuelve"] = alloc_view["Deuda Resuelve"].map(_format_currency0)
    alloc_view["Pago Banco Total"] = alloc_view["Pago Banco Total"].map(_format_currency0)
    st.caption("Arriba solo define cómo repartir Pago Banco Total y N PaB por deuda; abajo editas cada flujo completo.")
    ...
    pago_banco_edit = [max(float(_parse_amount_input(x)), 0.0) for x in edited_alloc["Pago Banco Total"].tolist()]
    ...
    n_pab_edit = [max(int(_parse_amount_input(x, max_decimals=0)), 1) for x in edited_alloc["N PaB"].tolist()]
    ...
    primeras_com_edit = ce_inicial_default
