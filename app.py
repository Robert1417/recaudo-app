...
if st.session_state.get("modo_sin_portafolio", False) and len(ids_sel) > 1:
    ...
    npab_view = deuda_detalle[["Id deuda", "Deuda Resuelve"]].copy()
    npab_view["N PaB"] = n_pab_deuda
    npab_view["Deuda Resuelve"] = npab_view["Deuda Resuelve"].map(_format_currency0)
    st.caption("Distribuye únicamente el N PaB por deuda (mínimo 1 por deuda y la suma debe ser igual al N PaB global).")
    edited_npab = st.data_editor(
        npab_view,
        key=per_debt_key,
        use_container_width=True,
        hide_index=True,
        num_rows="fixed",
        disabled=["Id deuda", "Deuda Resuelve"],
    )
    n_pab_edit = [max(int(_parse_amount_input(x, max_decimals=0)), 1) for x in edited_npab["N PaB"].tolist()]
    diff_npab = int(n_pab) - sum(n_pab_edit)
    if n_pab_edit and diff_npab != 0:
        idx_ajuste = 0 if diff_npab > 0 else int(np.argmax(n_pab_edit))
        n_pab_edit[idx_ajuste] = max(1, n_pab_edit[idx_ajuste] + diff_npab)
    if sum(n_pab_edit) != int(n_pab):
        st.warning("La suma de N PaB por deuda debe coincidir con el N PaB global; se ajustó automáticamente.")
        n_pab_edit = _split_integer_equitable(n_pab, len(deuda_detalle))

    pago_keys = [f"sin_port_pago_total_{ref_input}_{deuda_id}" for deuda_id in deuda_detalle["Id deuda"].tolist()]
    for i, pago_key in enumerate(pago_keys):
        if pago_key not in st.session_state:
            st.session_state[pago_key] = float(pago_banco_deuda[i])
    pago_banco_edit = [float(_parse_amount_input(st.session_state.get(k, 0.0))) for k in pago_keys]
    default_pagos = [float(x) for x in pago_banco_deuda]
    ...
    for i, k in enumerate(pago_keys):
        if not pago_edit_flags[i]:
            st.session_state[k] = float(pago_banco_edit[i])
    ...
    ce_keys = [f"sin_port_ce_inicial_{ref_input}_{deuda_id}" for deuda_id in deuda_detalle["Id deuda"].tolist()]
    for i, ce_key in enumerate(ce_keys):
        if ce_key not in st.session_state:
            st.session_state[ce_key] = float(ce_inicial_default[i])
    primeras_com_edit = [float(_parse_amount_input(st.session_state.get(k, 0.0))) for k in ce_keys]
    ...
    for i, k in enumerate(ce_keys):
        if not ce_edit_flags[i]:
            st.session_state[k] = float(primeras_com_edit[i])

    default_conf = pd.DataFrame({
        "Id deuda": deuda_detalle["Id deuda"],
        "Deuda Resuelve": deuda_detalle["Deuda Resuelve"],
        "Pago Banco Total": pago_banco_edit,
        "Comisión Total": comision_deuda_calc,
        "CE Inicial": primeras_com_edit,
        "N PaB": n_pab_edit,
    })
    ...
    for i, row in default_conf.iterrows():
        ...
        with c1:
            pago_widget_value = min(max(float(_parse_amount_input(st.session_state.get(f"sin_port_pago_total_{ref_input}_{deuda_id}", pago_total_i))), 0.0), float(pago_banco))
            st.number_input(
                f"Pago Banco Total · {deuda_id}",
                min_value=0.0,
                max_value=float(pago_banco),
                value=pago_widget_value,
                step=0.0,
                format="%.0f",
                key=f"sin_port_pago_total_{ref_input}_{deuda_id}",
            )
        with c2:
            primer_widget_value = min(max(float(_parse_amount_input(st.session_state.get(primer_banco_key, primer_banco_i))), 0.0), pago_total_i)
            st.number_input(
                f"Primer Pago Banco · {deuda_id}",
                min_value=0.0,
                max_value=pago_total_i,
                value=primer_widget_value,
                step=0.0,
                format="%.0f",
                key=primer_banco_key,
            )
        with c3:
            ce_widget_key = f"sin_port_ce_inicial_{ref_input}_{deuda_id}"
            ce_widget_value = min(max(float(_parse_amount_input(st.session_state.get(ce_widget_key, ce_inicial_i))), 0.0), comision_total_i)
            st.number_input(
                f"CE Inicial · {deuda_id}",
                min_value=0.0,
                max_value=comision_total_i,
                value=ce_widget_value,
                step=0.0,
                format="%.0f",
                key=ce_widget_key,
            )
        st.caption(f"Sugerido por proporción: Pago Banco {_format_currency0(default_pagos[i])} · CE Inicial {_format_currency0(ce_default_vals[i])}")
