...
if st.session_state.get("modo_sin_portafolio", False) and len(ids_sel) > 1:
    ...
    for i, row in default_conf.iterrows():
        ...
        if primer_banco_key not in st.session_state:
            st.session_state[primer_banco_key] = float(min(primer_pago_banco * (float(row["Pago Banco Total"]) / float(max(pago_banco, 1.0))), float(row["Pago Banco Total"])))
        primer_banco_i = min(max(float(_parse_amount_input(st.session_state.get(primer_banco_key, 0.0))), 0.0), float(row["Pago Banco Total"]))
        primera_com_i = min(max(float(row["CE Inicial"]), 0.0), float(row["Comisión Total"]))
        ...
        with c1:
            st.number_input(
                ...
                value=float(row["Pago Banco Total"]),
                step=0.0,
                format="%.0f",
        ...
        with c2:
            st.number_input(
                ...
                min_value=0.0,
                max_value=float(row["Pago Banco Total"]),
                value=float(primer_banco_i),
                step=0.0,
                format="%.0f",
        ...
        with c3:
            st.number_input(
                ...
                min_value=0.0,
                max_value=float(row["Comisión Total"]),
                value=float(row["CE Inicial"]),
                step=0.0,
                format="%.0f",
                key=f"sin_port_ce_inicial_{ref_input}_{deuda_id}",
            )
