...
def _build_liquidacion_sin_portafolio(
    ...
    for idx_deuda, deuda_id in enumerate(ids_sel, start=1):
        ...
        with st.expander(f"Deuda {idx_deuda}: ID {deuda_id} • {banco}", expanded=(idx_deuda == 1)):
            ...
            prev_auto_ce = float(st.session_state.get(f"{base_key}_comision_exito_auto", auto_ce) or auto_ce)
            current_ce_state = float(st.session_state.get(f"{base_key}_comision_exito", auto_ce) or auto_ce)
            st.session_state[f"{base_key}_comision_exito_auto"] = auto_ce

            cambian_derivados = (abs(prev_pab - pago_banco) > 0.5) or (prev_n != n_pab)
            if cambian_derivados and abs(current_ce_state - prev_auto_ce) <= 0.5:
                st.session_state[f"{base_key}_comision_exito_overridden"] = False
            if cambian_derivados and not st.session_state.get(f"{base_key}_comision_exito_overridden", False):
            ...
            with c4:
                ...
                if not st.session_state.get(f"{base_key}_comision_exito_overridden", False):
                    if abs(float(ce_actual or 0.0) - float(auto_ce or 0.0)) > 0.5:
                        st.session_state[f"{base_key}_comision_exito_overridden"] = True
            with c5:
