...
def _build_bulk_template_df() -> pd.DataFrame:
    """Devuelve la plantilla exacta que espera la carga masiva."""
    return pd.DataFrame(columns=BULK_INPUT_COLUMNS)


def _dataframe_to_excel_bytes(df: pd.DataFrame, sheet_name: str = "predicciones") -> bytes:
    output = BytesIO()
...
def _read_bulk_prediction_file(uploaded_file) -> pd.DataFrame:
...
def run_bulk_predictions(df_input: pd.DataFrame, cartera_df: pd.DataFrame | None = None) -> tuple[pd.DataFrame, dict]:
    ...
    for row_number, row in enumerate(df_result.to_dict(orient="records"), start=2):
        case = _extract_case_data_from_record(row)
        try:
            referencia = str(case.get("referencia", "")).strip()
            if not referencia:
            ...
            pred, _low_ratio_cap_applied = _predict_recaudo_result(model, features)
            umbral = 0.8 if _is_traditional_liquidation(tipo_liquidacion) else 0.74
            aprobado = float(pred) >= float(umbral)
...
def main():
    ...
    if source_mode == "Masiva (CSV/XLSX)":
        ...
        template_df = _build_bulk_template_df()
        c_template_xlsx, c_template_csv = st.columns(2)
        with c_template_xlsx:
            st.download_button(
                "⬇️ Descargar plantilla Excel",
                data=_dataframe_to_excel_bytes(template_df, sheet_name="plantilla"),
        ...
        if bulk_file is not None:
            try:
                df_bulk = _read_bulk_prediction_file(bulk_file)
                ...
                if st.button("🔮 Calcular predicciones masivas", type="primary", use_container_width=True):
                    df_pred, bulk_summary = run_bulk_predictions(df_bulk, cartera_df=cartera_df)
                    st.success(
                        f"Predicciones calculadas: {bulk_summary['ok']:,} de {bulk_summary['total']:,} filas."
                    )
                    ...
                    st.download_button(
                        ...
                        data=_dataframe_to_excel_bytes(df_pred, sheet_name="predicciones"),
